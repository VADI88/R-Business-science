...
```{r}
# STOCK PRICES
sp_500_prices_tbl <- read_rds("../data/week_6_data/sp_500_prices_tbl.rds")
sp_500_prices_tbl
```
...
```{r}
# SECTOR INFORMATION
sp_500_index_tbl <- read_rds("../data/week_6_data/sp_500_index_tbl.rds")
sp_500_index_tbl
```
...
sp_500_daily_returns_tbl <- sp_500_prices_tbl |> 
    ...
    group_by(symbol) |> 
    mutate(adjusted_lag1 = adjusted |> lag(1)) |>
    filter(!is.na(adjusted_lag1)) |> 
    mutate(difference = adjusted - adjusted_lag1) |> 
    mutate(pct_return = difference/ adjusted_lag1) |> 
...
```{r}
sp_500_daily_returns_tbl <- read_rds("../data/week_6_data/sp_500_daily_returns_tbl.rds")
...
```{r}
stock_date_matrix_tbl <- read_rds("../data/week_6_data/stock_date_matrix_tbl.rds")
```
...
```{r}
kmeans_mapper <- function(data,center = 3) {
    data |>
...
kmeans_center <- tibble(center = 1:30)


k_means_mapped_tbl <- kmeans_center |>
  mutate(
    kmeans_obj = center |>
      map(~ kmeans_mapper(data = sp_500_daily_returns_tbl, center = .x))
  ) |>
...
```{r}
k_means_mapped_tbl <- read_rds("../data/week_6_data/k_means_mapped_tbl.rds")
```
...
```{r}
k_means_mapped_tbl <- read_rds("../data/week_6_data/k_means_mapped_tbl.rds")
umap_results_tbl   <- read_rds("../data/week_6_data/umap_results_tbl.rds")
```
