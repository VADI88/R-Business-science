...
sales_by_year_category_2_tbl |>
  mutate(category_2_num = category_2 |> as.numeric())
...
sales_by_year_category_2_tbl |>
  ...
  geom_point(color = 'tomato3', size = 5)
...
sales_by_year_category_2_tbl |>
  ggplot(aes(x = category_2, y = revenue, fill = year)) +
  geom_col(color = 'white', position = 'dodge2')
